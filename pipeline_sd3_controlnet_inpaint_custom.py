...
class SD3_ControlNet_Custom_Inpainting_Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    r"""
    Args:
    ...
    def __init__(
        ...
        self.image_processor_1 = VaeImageProcessor(
            vae_scale_factor=self.vae_scale_factor, do_resize=True, do_convert_rgb=True, do_normalize=True
        )
        self.image_processor_2 = VaeImageProcessor(vae_scale_factor=self.vae_scale_factor)
    ...
    def prepare_image(
        ...
        if isinstance(image, torch.Tensor):
            ...
            image = self.image_processor_2.preprocess(image, height=height, width=width)
    ...
    def prepare_image_with_mask(
        ...
        if isinstance(image, torch.Tensor):
            ...
            image = self.image_processor_1.preprocess(image, height=height, width=width)
    ...
    def __call__(
        ...
        if isinstance(self.controlnet, SD3ControlNetModel):
            ...
            ## First controlnet is used for inpainting

            control_images = []

            for i in range(len(control_image)):
                if control_mask[i] is not None:
                    # this is for inpainting controlnet
                    control_image_ = self.prepare_image_with_mask(
                        image=control_image[i],
                        mask=control_mask[i],
                        width=width,
                        height=height,
                        batch_size=batch_size * num_images_per_prompt,
            ...
            # control_image_1 = self.prepare_image_with_mask(
            ...
            # ## Second controlnet is used for pose control
            # control_image_2 = self.prepare_image(
            #     image=control_image[1],
            #     width=width,
            #     height=height,
            #     batch_size=batch_size * num_images_per_prompt,
            #     num_images_per_prompt=num_images_per_prompt,
            ...
            latent_height, latent_width = control_image[0].shape[-2:]
            height = latent_height * self.vae_scale_factor
            width = latent_width * self.vae_scale_factor
        else:
            assert False
        ...
        if output_type == "latent":
            ...
            image = self.image_processor_1.postprocess(image, output_type=output_type)
